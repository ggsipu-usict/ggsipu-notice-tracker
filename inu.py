...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3945.16 Safari/537.36"
}
...
def get_notices(soup, url_prefix=""):
    ...
    for f_tr in f_trs:
        ...
        if notice:
            notice["url"] = urljoin(url_prefix, notice["url"])
            yield notice
...
class Telegram(Dispatcher):
    ...
    @classmethod
    def sendMessage(
        cls, msg, markdownv2=True, web_page_preview=False, reply_markup=None, **kwargs
    ):
        data = {
    ...
    def send_doc_from_url(cls, url, caption=None, markdownv2=True, **kwargs):
        return (
            cls.sendDocument(None, url, caption, markdownv2, **kwargs).status_code
            == 200
        )
...
class Source(metaclass=InstanceReprMixin):
    ...
    def __init__(self):
        ...
        self.failed_path = path.join("dump", self.__source_name__, "failed.yml")
